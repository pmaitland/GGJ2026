...
func get_disabled_cell_neighbour_bitmask(cell: Vector2) -> String:
	var bits: String = ""
	if _a_star.is_point_disabled(_a_star.get_closest_point(Vector2(cell.x, cell.y-1), true)):
		bits += "1"
	else:
		bits += "0"
	if _a_star.is_point_disabled(_a_star.get_closest_point(Vector2(cell.x+1, cell.y-1), true)):
		bits += "1"
	else:
		bits += "0"
	if _a_star.is_point_disabled(_a_star.get_closest_point(Vector2(cell.x+1, cell.y), true)):
		bits += "1"
	else:
		bits += "0"
	if _a_star.is_point_disabled(_a_star.get_closest_point(Vector2(cell.x+1, cell.y+1), true)):
